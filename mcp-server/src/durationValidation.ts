...
export interface DurationViolation {
  ...
  requiredMinimum?: Duration;
  requiredMaximum?: Duration;
}
 ...
 * Valid range: [parent * 0.1, parent)
 */
export function validateSegmentDuration(
  segmentDuration: Duration,
  parentDuration: Duration,
  segmentId: TemplateId,
  segmentIntent: string,
 ...
 */
export function validateLaneSegmentDurations(
  lane: LaneTemplate,
  templateMap: Map<TemplateId, Template>
): DurationValidationResult {
  const violations: DurationViolation[] = [];
  ...
  for (const segment of lane.segments) {
    ...
    const result = validateSegmentDuration(
      childTemplate.estimatedDuration,
      lane.estimatedDuration,
      childTemplate.id,
      childTemplate.intent,
      lane.id,
      lane.intent
    );
...
export function validateTemplateReferences(
  ...
  for (const parentRef of template.references) {
    ...
    const parentResult = validateTemplateReferences(
      parentTemplate,
      parentTemplate.estimatedDuration,
      templateMap
    );
 ...
 */
export function validateLaneDurationChange(
  lane: LaneTemplate,
  newDuration: Duration,
  templateMap: Map<TemplateId, Template>
): DurationValidationResult {
 ...
 */
function formatDuration(ms: Duration): string {
 ...
 */
export function formatDurationViolationErrors(violations: DurationViolation[]): string[] {
  return violations.map(v => {
    if (v.type === 'too-small') {
      return (
        `Segment "${v.segmentIntent}" (${formatDuration(v.segmentDuration)}) does not meet the minimum ` +
      ...
      return (
        `Segment "${v.segmentIntent}" (${formatDuration(v.segmentDuration)}) is too large for parent ` +
