...
server.tool(
  'analyze_process_intent',
  'Analyze a natural language process description and suggest appropriate input/output variables. This is domain-agnostic and works for any process (cooking, manufacturing, software development, etc.). Returns suggestions that can be used with create_busy_template or create_lane_template.',
  {
  ...
  async ({ processDescription, domain }) => {
    ...
    const suggestions: {
      suggestedInputs: Array<{ variable: string; quantity: number; reasoning: string }>;
      suggestedOutputs: Array<{ variable: string; quantity: number; reasoning: string }>;
      domainInferred: string;
    ...
    const hasVerb = (verbs: string[]) => verbs.some(v => description.includes(v));
    const hasNoun = (nouns: string[]) => nouns.some(n => description.includes(n));
  ...
  async ({ templateId, newDuration }) => {
    ...
    try {
      ...
      const {
        ...
        formatDurationViolationErrors,
      } = await import('./durationValidation.js');
...
server.tool(
  'update_intent',
  'Update the intent (human-readable description) of any template. High specificity operation that only changes the intent field.',
  {
    templateId: z.string().describe('The ID of the template to update'),
...
server.tool(
  'update_consumption_variables',
  'Update the willConsume variables for a busy template. This operation replaces the entire willConsume ledger. Use this to change what inputs a busy template requires.',
  {
...
server.tool(
  ...
  'Update the willProduce variables for a busy template. This operation replaces the entire willProduce ledger. Use this to change what outputs a busy template generates.',
  {
    templateId: z.string().describe('The ID of the busy template to update'),
