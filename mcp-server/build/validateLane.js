 ...
 */
function resolveFirstBusy(template, allTemplates, parentOffset = 0) {
 ...
 */
function resolveLastBusy(template, allTemplates, parentOffset = 0) {
    if (template.templateType === 'busy') {
        return {
            busy: template,
 ...
 */
function flattenBusyTemplates(template, allTemplates, parentOffset = 0, errors = []) {
    if (template.templateType === 'busy') {
        return [{
                template,
    ...
    for (const segment of lane.segments) {
        ...
        const absoluteOffset = parentOffset + segment.offset;
        result.push(...flattenBusyTemplates(segmentTemplate, allTemplates, absoluteOffset, errors));
    }
    return result;
}
// ============================================================================
 ...
 */
function checkBusyOverlaps(flattened) {
    const errors = [];
 ...
 */
function simulateLedger(flattened, firstBusy, lastBusy) {
    const errors = [];
    ...
    for (const [varName, producedQty] of Object.entries(allProduced)) {
        ...
        if (remaining > 0) {
            ...
            const producerItems = sorted.filter((item) => Object.keys(item.template.willProduce).includes(varName));
 ...
 */
export function validateLane(lane, allTemplates) {
    const errors = [];
 ...
 */
export function validateBusy(busy) {
    return {
        isValid: true,
        errors: [],
        firstBusy: busy,
 ...
 */
export function validateTemplate(template, allTemplates) {
    if (template.templateType === 'busy') {
